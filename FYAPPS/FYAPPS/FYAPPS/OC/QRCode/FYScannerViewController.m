...
@interface FYScannerViewController () <UIImagePickerControllerDelegate, UINavigationControllerDelegate>
/// 名片字符串
...
@implementation FYScannerViewController {
    /// 扫描框
    FYScannerBorder *scannerBorder;
    /// 扫描器
    FYScanner *scanner;
    /// 提示标签
    UILabel *tipLabel;
...
- (void)viewDidLoad {
    ...
    scanner = [FYScanner scanerWithView:self.view scanFrame:scannerBorder.frame completion:^(NSString *stringValue) {
        // 完成回调
...
- (void)clickCardButton {
    FYScanerCardViewController *vc = [[FYScanerCardViewController alloc] initWithCardName:self.cardName avatar:self.avatar];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    // 扫描图像
    [FYScanner scaneImage:image completion:^(NSArray *values) {
...
- (void)prepareScanerBorder {
    ...
    scannerBorder = [[FYScannerBorder alloc] initWithFrame:CGRectMake(0, 0, width, width)];
    ...
    FYScannerMaskView *maskView = [FYScannerMaskView maskViewWithFrame:self.view.bounds cropRect:scannerBorder.frame];
