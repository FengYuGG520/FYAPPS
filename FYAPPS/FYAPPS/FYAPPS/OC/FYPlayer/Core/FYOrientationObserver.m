...
@interface FYOrientationObserver ()
...
@property (nonatomic, assign) FYRotateType roateType;

@property (nonatomic, strong) UIView *blackView;

@end

@implementation FYOrientationObserver

- (instancetype)init {
    self = [super init];
    if (self) {
        _duration = 0.30;
        _fullScreenMode = FYFullScreenModeLandscape;
        _supportInterfaceOrientation = FYInterfaceOrientationMaskAllButUpsideDown;
        _allowOrentitaionRotation = YES;
        _roateType = FYRotateTypeNormal;
    }
    return self;
}
...
- (void)cellModelRotateView:(UIView *)rotateView rotateViewAtCell:(UIView *)cell playerViewTag:(NSInteger)playerViewTag {
    self.roateType = FYRotateTypeCell;
    self.view = rotateView;
    self.cell = cell;
...
- (void)cellOtherModelRotateView:(UIView *)rotateView containerView:(UIView *)containerView {
    self.roateType = FYRotateTypeCellOther;
    self.view = rotateView;
    self.containerView = containerView;
}
...
- (void)handleDeviceOrientationChange {
    if (self.fullScreenMode == FYFullScreenModePortrait || !self.allowOrentitaionRotation) return;
...
- (void)enterLandscapeFullScreen:(UIInterfaceOrientation)orientation animated:(BOOL)animated {
    if (self.fullScreenMode == FYFullScreenModePortrait) return;
    ...
    if ([self isNeedAdaptiveiOS8Rotation] || self.forceDeviceOrientation) {
        if (UIInterfaceOrientationIsLandscape(orientation)) {
            ...
            if (!self.fullScreen) return;
            if (self.roateType == FYRotateTypeCell) superview = [self.cell viewWithTag:self.playerViewTag];
...
#pragma clang diagnostic pop
    ...
    if (SysVersion >= 8.0 && SysVersion < 9.0) {
        ...
        if(windowCount > 1) {
            ...
            if (UIInterfaceOrientationIsLandscape(orientation)) {
                keyboardWindow.bounds = CGRectMake(0, 0, MAX(FYPlayerScreenHeight, FYPlayerScreenWidth), MIN(FYPlayerScreenHeight, FYPlayerScreenWidth));
            } else {
                keyboardWindow.bounds = CGRectMake(0, 0, MIN(FYPlayerScreenHeight, FYPlayerScreenWidth), MAX(FYPlayerScreenHeight, FYPlayerScreenWidth));
            }
...
- (void)enterPortraitFullScreen:(BOOL)fullScreen animated:(BOOL)animated {
    if (self.fullScreenMode == FYFullScreenModeLandscape) return;
    UIView *superview = nil;
    if (fullScreen) {
        superview = self.fullScreenContainerView;
        self.view.frame = [self.view convertRect:self.view.frame toView:superview];
        ...
        if (self.roateType == FYRotateTypeCell) {
...
- (void)exitFullScreenWithAnimated:(BOOL)animated {
    if (self.fullScreenMode == FYFullScreenModeLandscape) {
        [self enterLandscapeFullScreen:UIInterfaceOrientationPortrait animated:animated];
    } else if (self.fullScreenMode == FYFullScreenModePortrait) {
...
- (BOOL)isSupportedPortrait {
    return self.supportInterfaceOrientation & FYInterfaceOrientationMaskPortrait;
}

/// 是否支持 LandscapeLeft
- (BOOL)isSupportedLandscapeLeft {
    return self.supportInterfaceOrientation & FYInterfaceOrientationMaskLandscapeLeft;
}

/// 是否支持 LandscapeRight
- (BOOL)isSupportedLandscapeRight {
    return self.supportInterfaceOrientation & FYInterfaceOrientationMaskLandscapeRight;
}
...
- (void)setFullScreen:(BOOL)fullScreen {
    ...
    [[UIWindow fy_currentViewController] setNeedsStatusBarAppearanceUpdate];
}
