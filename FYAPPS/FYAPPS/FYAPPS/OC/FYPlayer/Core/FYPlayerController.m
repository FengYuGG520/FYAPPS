...
@interface FYPlayerController ()

@property (nonatomic, strong) FYPlayerNotification *notification;
@property (nonatomic, weak) UIScrollView *scrollView;
@property (nonatomic, strong) UISlider *volumeViewSlider;
@property (nonatomic, assign) NSInteger containerViewTag;
@property (nonatomic, assign) FYPlayerContainerType containerType;
/// The player's small container view.
@property (nonatomic, strong) FYFloatView *smallFloatView;
...
@implementation FYPlayerController

- (instancetype)init {
    self = [super init];
    if (self) {
        @weakify(self)
        [[FYReachabilityManager sharedManager] startMonitoring];
        [[FYReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(FYReachabilityStatus status) {
            @strongify(self)
...
+ (instancetype)playerWithPlayerManager:(id<FYPlayerMediaPlayback>)playerManager containerView:(nonnull UIView *)containerView {
    FYPlayerController *player = [[self alloc] initWithPlayerManager:playerManager containerView:containerView];
    return player;
}
...
+ (instancetype)playerWithScrollView:(UIScrollView *)scrollView playerManager:(id<FYPlayerMediaPlayback>)playerManager containerViewTag:(NSInteger)containerViewTag {
    FYPlayerController *player = [[self alloc] initWithScrollView:scrollView playerManager:playerManager containerViewTag:containerViewTag];
    return player;
}
...
+ (instancetype)playerWithScrollView:(UIScrollView *)scrollView playerManager:(id<FYPlayerMediaPlayback>)playerManager containerView:(UIView *)containerView {
    FYPlayerController *player = [[self alloc] initWithScrollView:scrollView playerManager:playerManager containerView:containerView];
    return player;
}

- (instancetype)initWithPlayerManager:(id<FYPlayerMediaPlayback>)playerManager containerView:(nonnull UIView *)containerView {
    FYPlayerController *player = [self init];
    player.containerView = containerView;
    player.currentPlayerManager = playerManager;
    player.containerType = FYPlayerContainerTypeView;
    return player;
}
...
- (instancetype)initWithScrollView:(UIScrollView *)scrollView playerManager:(id<FYPlayerMediaPlayback>)playerManager containerViewTag:(NSInteger)containerViewTag {
    ...
    player.currentPlayerManager = playerManager;
    player.containerType = FYPlayerContainerTypeCell;
    return player;
}
...
- (void)playerManagerCallbcak {
    @weakify(self)
    self.currentPlayerManager.playerPrepareToPlay = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, NSURL * _Nonnull assetURL) {
        @strongify(self)
        self.currentPlayerManager.view.hidden = NO;
        [self.notification addNotification];
        [self addDeviceOrientationObserver];
        if (self.scrollView) {
            self.scrollView.fy_stopPlay = NO;
        }
    ...
    self.currentPlayerManager.playerReadyToPlay = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, NSURL * _Nonnull assetURL) {
        @strongify(self)
    ...
    self.currentPlayerManager.playerPlayTimeChanged = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, NSTimeInterval currentTime, NSTimeInterval duration) {
        @strongify(self)
    ...
    self.currentPlayerManager.playerBufferTimeChanged = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, NSTimeInterval bufferTime) {
        @strongify(self)
    ...
    self.currentPlayerManager.playerPlayStateChanged = ^(id  _Nonnull asset, FYPlayerPlaybackState playState) {
        @strongify(self)
    ...
    self.currentPlayerManager.playerLoadStateChanged = ^(id  _Nonnull asset, FYPlayerLoadState loadState) {
        @strongify(self)
    ...
    self.currentPlayerManager.playerPlayFailed = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, id  _Nonnull error) {
        @strongify(self)
    ...
    self.currentPlayerManager.presentationSizeChanged = ^(id<FYPlayerMediaPlayback>  _Nonnull asset, CGSize size){
        @strongify(self)
        if (self.orientationObserver.fullScreenMode == FYFullScreenModeAutomatic) {
            if (size.width > size.height) {
                self.orientationObserver.fullScreenMode = FYFullScreenModeLandscape;
            } else {
                self.orientationObserver.fullScreenMode = FYFullScreenModePortrait;
            }
        }
...
- (FYPlayerNotification *)notification {
    if (!_notification) {
        _notification = [[FYPlayerNotification alloc] init];
        @weakify(self)
        _notification.willResignActive = ^(FYPlayerNotification * _Nonnull registrar) {
            @strongify(self)
        ...
        _notification.didBecomeActive = ^(FYPlayerNotification * _Nonnull registrar) {
            @strongify(self)
            if (self.isViewControllerDisappear) return;
        ...
        _notification.oldDeviceUnavailable = ^(FYPlayerNotification * _Nonnull registrar) {
            @strongify(self)
...
- (FYFloatView *)smallFloatView {
    if (!_smallFloatView) {
        _smallFloatView = [[FYFloatView alloc] init];
...
- (void)setCurrentPlayerManager:(id<FYPlayerMediaPlayback>)currentPlayerManager {
...
- (void)setContainerView:(UIView *)containerView {
    ...
    if (self.scrollView) {
        self.scrollView.fy_containerView = containerView;
    }
...
- (void)setControlView:(UIView<FYPlayerMediaControl> *)controlView {
...
- (void)setContainerType:(FYPlayerContainerType)containerType {
    _containerType = containerType;
    if (self.scrollView) {
        self.scrollView.fy_containerType = containerType;
    }
}

@end

@implementation FYPlayerController (FYPlayerTimeControl)
...
@implementation FYPlayerController (FYPlayerPlaybackControl)
...
- (void)stop {
    ...
    if (self.scrollView) {
        self.scrollView.fy_stopPlay = YES;
    }
}

- (void)replaceCurrentPlayerManager:(id<FYPlayerMediaPlayback>)playerManager {
...
- (void)addPlayerViewToCell {
    ...
    UIView *cell = [self.scrollView fy_getCellForIndexPath:self.playingIndexPath];
...
- (void)stopCurrentPlayingCell {
    if (self.scrollView.fy_playingIndexPath) {
        [self stop];
        self.isSmallFloatViewShow = NO;
...
- (FYPlayerPlaybackState)playState {
...
- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSURL * _Nonnull))playerPrepareToPlay {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSURL * _Nonnull))playerReadyToPlay {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSTimeInterval, NSTimeInterval))playerPlayTimeChanged {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSTimeInterval))playerBufferTimeChanged {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, FYPlayerPlaybackState))playerPlayStateChanged {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, FYPlayerLoadState))playerLoadStateChanged {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull))playerDidToEnd {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, id _Nonnull))playerPlayFailed {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(id<FYPlayerMediaPlayback> _Nonnull, CGSize ))presentationSizeChanged {
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setPlayerPrepareToPlay:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSURL * _Nonnull))playerPrepareToPlay {
    objc_setAssociatedObject(self, @selector(playerPrepareToPlay), playerPrepareToPlay, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerReadyToPlay:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSURL * _Nonnull))playerReadyToPlay {
    objc_setAssociatedObject(self, @selector(playerReadyToPlay), playerReadyToPlay, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerPlayTimeChanged:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSTimeInterval, NSTimeInterval))playerPlayTimeChanged {
    objc_setAssociatedObject(self, @selector(playerPlayTimeChanged), playerPlayTimeChanged, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerBufferTimeChanged:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, NSTimeInterval))playerBufferTimeChanged {
    objc_setAssociatedObject(self, @selector(playerBufferTimeChanged), playerBufferTimeChanged, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerPlayStateChanged:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, FYPlayerPlaybackState))playerPlayStateChanged {
    objc_setAssociatedObject(self, @selector(playerPlayStateChanged), playerPlayStateChanged, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerLoadStateChanged:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, FYPlayerLoadState))playerLoadStateChanged {
    objc_setAssociatedObject(self, @selector(playerLoadStateChanged), playerLoadStateChanged, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerDidToEnd:(void (^)(id<FYPlayerMediaPlayback> _Nonnull))playerDidToEnd {
    objc_setAssociatedObject(self, @selector(playerDidToEnd), playerDidToEnd, OBJC_ASSOCIATION_COPY);
}

- (void)setPlayerPlayFailed:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, id _Nonnull))playerPlayFailed {
    objc_setAssociatedObject(self, @selector(playerPlayFailed), playerPlayFailed, OBJC_ASSOCIATION_COPY);
}

- (void)setPresentationSizeChanged:(void (^)(id<FYPlayerMediaPlayback> _Nonnull, CGSize))presentationSizeChanged {
...
- (void)setViewControllerDisappear:(BOOL)viewControllerDisappear {
    ...
    if (self.scrollView) self.scrollView.fy_viewControllerDisappear = viewControllerDisappear;
...
@implementation FYPlayerController (FYPlayerOrientationRotation)
...
- (void)enterFullScreen:(BOOL)fullScreen animated:(BOOL)animated {
    if (self.orientationObserver.fullScreenMode == FYFullScreenModePortrait) {
        [self.orientationObserver enterPortraitFullScreen:fullScreen animated:YES];
    } else {
...
- (FYOrientationObserver *)orientationObserver {
    @weakify(self)
    FYOrientationObserver *orientationObserver = objc_getAssociatedObject(self, _cmd);
    if (!orientationObserver) {
        orientationObserver = [[FYOrientationObserver alloc] init];
        orientationObserver.orientationWillChange = ^(FYOrientationObserver * _Nonnull observer, BOOL isFullScreen) {
            @strongify(self)
        ...
        orientationObserver.orientationDidChanged = ^(FYOrientationObserver * _Nonnull observer, BOOL isFullScreen) {
            @strongify(self)
...
- (void (^)(FYPlayerController * _Nonnull, BOOL))orientationWillChange {
    return objc_getAssociatedObject(self, _cmd);
}

- (void (^)(FYPlayerController * _Nonnull, BOOL))orientationDidChanged {
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setOrientationWillChange:(void (^)(FYPlayerController * _Nonnull, BOOL))orientationWillChange {
    objc_setAssociatedObject(self, @selector(orientationWillChange), orientationWillChange, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (void)setOrientationDidChanged:(void (^)(FYPlayerController * _Nonnull, BOOL))orientationDidChanged {
...
@implementation FYPlayerController (FYPlayerViewGesture)

#pragma mark - getter

- (FYPlayerGestureControl *)gestureControl {
    FYPlayerGestureControl *gestureControl = objc_getAssociatedObject(self, _cmd);
    if (!gestureControl) {
        gestureControl = [[FYPlayerGestureControl alloc] init];
        @weakify(self)
        gestureControl.triggerCondition = ^BOOL(FYPlayerGestureControl * _Nonnull control, FYPlayerGestureType type, UIGestureRecognizer * _Nonnull gesture, UITouch *touch) {
            @strongify(self)
        ...
        gestureControl.singleTapped = ^(FYPlayerGestureControl * _Nonnull control) {
            @strongify(self)
        ...
        gestureControl.doubleTapped = ^(FYPlayerGestureControl * _Nonnull control) {
            @strongify(self)
        ...
        gestureControl.beganPan = ^(FYPlayerGestureControl * _Nonnull control, FYPanDirection direction, FYPanLocation location) {
            @strongify(self)
        ...
        gestureControl.changedPan = ^(FYPlayerGestureControl * _Nonnull control, FYPanDirection direction, FYPanLocation location, CGPoint velocity) {
            @strongify(self)
        ...
        gestureControl.endedPan = ^(FYPlayerGestureControl * _Nonnull control, FYPanDirection direction, FYPanLocation location) {
            @strongify(self)
        ...
        gestureControl.pinched = ^(FYPlayerGestureControl * _Nonnull control, float scale) {
            @strongify(self)
...
- (FYPlayerDisableGestureTypes)disableGestureTypes {
    return [objc_getAssociatedObject(self, _cmd) integerValue];
}

- (FYPlayerDisablePanMovingDirection)disablePanMovingDirection {
    return [objc_getAssociatedObject(self, _cmd) integerValue];
}

#pragma mark - setter

- (void)setDisableGestureTypes:(FYPlayerDisableGestureTypes)disableGestureTypes {
...
- (void)setDisablePanMovingDirection:(FYPlayerDisablePanMovingDirection)disablePanMovingDirection {
...
@implementation FYPlayerController (FYPlayerScrollView)
...
- (void)setScrollView:(UIScrollView *)scrollView {
    ...
    self.scrollView.fy_WWANAutoPlay = self.isWWANAutoPlay;
    @weakify(self)
    scrollView.fy_playerWillAppearInScrollView = ^(NSIndexPath * _Nonnull indexPath) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerWillAppearInScrollView) self.fy_playerWillAppearInScrollView(indexPath);
    ...
    scrollView.fy_playerDidAppearInScrollView = ^(NSIndexPath * _Nonnull indexPath) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerDidAppearInScrollView) self.fy_playerDidAppearInScrollView(indexPath);
        if ([self.controlView respondsToSelector:@selector(playerDidAppearInScrollView:)]) {
            [self.controlView playerDidAppearInScrollView:self];
        }
    };
    
    scrollView.fy_playerWillDisappearInScrollView = ^(NSIndexPath * _Nonnull indexPath) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerWillDisappearInScrollView) self.fy_playerWillDisappearInScrollView(indexPath);
    ...
    scrollView.fy_playerDidDisappearInScrollView = ^(NSIndexPath * _Nonnull indexPath) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerDidDisappearInScrollView) self.fy_playerDidDisappearInScrollView(indexPath);
    ...
    scrollView.fy_playerAppearingInScrollView = ^(NSIndexPath * _Nonnull indexPath, CGFloat playerApperaPercent) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerAppearingInScrollView) self.fy_playerAppearingInScrollView(indexPath, playerApperaPercent);
        ...
        if (!self.stopWhileNotVisible && playerApperaPercent >= self.playerApperaPercent) {
            if (self.containerType == FYPlayerContainerTypeView) {
                [self addPlayerViewToContainerView:self.containerView];
            } else if (self.containerType == FYPlayerContainerTypeCell) {
                [self addPlayerViewToCell];
            }
        }
    };
    
    scrollView.fy_playerDisappearingInScrollView = ^(NSIndexPath * _Nonnull indexPath, CGFloat playerDisapperaPercent) {
        @strongify(self)
        if (self.isFullScreen) return;
        if (self.fy_playerDisappearingInScrollView) self.fy_playerDisappearingInScrollView(indexPath, playerDisapperaPercent);
...
- (void)setWWANAutoPlay:(BOOL)WWANAutoPlay {
    ...
    if (self.scrollView) self.scrollView.fy_WWANAutoPlay = self.isWWANAutoPlay;
}

- (void)setStopWhileNotVisible:(BOOL)stopWhileNotVisible {
    self.scrollView.fy_stopWhileNotVisible = stopWhileNotVisible;
...
- (void)setContainerViewTag:(NSInteger)containerViewTag {
    ...
    self.scrollView.fy_containerViewTag = containerViewTag;
}
...
- (void)setPlayingIndexPath:(NSIndexPath *)playingIndexPath {
    ...
    if (playingIndexPath) {
        [self stopCurrentPlayingCell];
        UIView *cell = [self.scrollView fy_getCellForIndexPath:playingIndexPath];
        self.containerView = [cell viewWithTag:self.containerViewTag];
        [self.orientationObserver cellModelRotateView:self.currentPlayerManager.view rotateViewAtCell:cell playerViewTag:self.containerViewTag];
        [self addDeviceOrientationObserver];
        self.scrollView.fy_playingIndexPath = playingIndexPath;
        [self layoutPlayerSubViews];
    } else {
        self.scrollView.fy_playingIndexPath = playingIndexPath;
    }
...
- (void)setShouldAutoPlay:(BOOL)shouldAutoPlay {
    ...
    self.scrollView.fy_shouldAutoPlay = shouldAutoPlay;
}
...
- (void)setPlayerDisapperaPercent:(CGFloat)playerDisapperaPercent {
    ...
    self.scrollView.fy_playerDisapperaPercent = playerDisapperaPercent;
...
- (void)setPlayerApperaPercent:(CGFloat)playerApperaPercent {
    ...
    self.scrollView.fy_playerApperaPercent = playerApperaPercent;
...
- (void)playTheIndexPath:(NSIndexPath *)indexPath scrollToTop:(BOOL)scrollToTop completionHandler:(void (^ _Nullable)(void))completionHandler {
    ...
    if (scrollToTop) {
        @weakify(self)
        [self.scrollView fy_scrollToRowAtIndexPath:indexPath completionHandler:^{
            @strongify(self)
...
- (void)playTheIndexPath:(NSIndexPath *)indexPath assetURL:(NSURL *)assetURL scrollToTop:(BOOL)scrollToTop {
    ...
    self.assetURL = assetURL;
    if (scrollToTop) {
        [self.scrollView fy_scrollToRowAtIndexPath:indexPath completionHandler:nil];
    }
}

@end

@implementation FYPlayerController (FYPlayerDeprecated)
