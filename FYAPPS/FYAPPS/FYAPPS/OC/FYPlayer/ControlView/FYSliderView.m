...
/** 滑块 */
@property (nonatomic, strong) FYSliderButton *sliderBtn;
...
- (void)layoutSubviews {
    ...
    self.sliderBtn.fy_centerX = self.bgProgressView.fy_width * self.value;
    
    min_x = 0;
    min_y = 0;
    if (self.sliderBtn.hidden) {
        min_w = self.bgProgressView.fy_width * self.value;
    } else {
        min_w = self.sliderBtn.fy_centerX;
    }
    min_h = self.sliderHeight;
    ...
    min_x = 0;
    min_y = 0;
    min_w = self.bgProgressView.fy_width * self.bufferValue;
    min_h = self.sliderHeight;
    ...
    self.bgProgressView.fy_centerY     = min_view_h * 0.5;
    self.bufferProgressView.fy_centerY = min_view_h * 0.5;
    self.sliderProgressView.fy_centerY = min_view_h * 0.5;
    self.sliderBtn.fy_centerY          = min_view_h * 0.5;
}
...
- (void)setValue:(float)value {
    ...
    _value = value;
    if (self.sliderBtn.hidden) {
        self.sliderProgressView.fy_width = self.bgProgressView.fy_width * value;
    } else {
        self.sliderBtn.fy_centerX = self.bgProgressView.fy_width * value;
        self.sliderProgressView.fy_width = self.sliderBtn.fy_centerX;
    }
}
...
- (void)setBufferValue:(float)bufferValue {
    ...
    self.bufferProgressView.fy_width = self.bgProgressView.fy_width * bufferValue;
}
...
- (void)setSliderHeight:(CGFloat)sliderHeight {
    ...
    self.bgProgressView.fy_height     = sliderHeight;
    self.bufferProgressView.fy_height = sliderHeight;
    self.sliderProgressView.fy_height = sliderHeight;
}
...
- (void)setIsHideSliderBlock:(BOOL)isHideSliderBlock {
    ...
    if (isHideSliderBlock) {
        ...
        self.bgProgressView.fy_left     = 0;
        self.bufferProgressView.fy_left = 0;
        self.sliderProgressView.fy_left = 0;
        self.allowTapped = NO;
    }
}
...
- (void)startAnimating {
    ...
    scaleAnimation.toValue = @(self.fy_width * 10);
...
- (void)sliderBtnDragMoving:(UIButton *)btn point:(CGPoint)touchPoint {
    ...
    CGFloat value = (point.x - btn.fy_width * 0.5) / self.bgProgressView.fy_width;
    // value的值需在0-1之间
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    // 获取进度
    CGFloat value = (point.x - self.sliderBtn.fy_width * 0.5) * 1.0 / self.bgProgressView.fy_width;
...
- (FYSliderButton *)sliderBtn {
    if (!_sliderBtn) {
        _sliderBtn = [FYSliderButton buttonWithType:UIButtonTypeCustom];
